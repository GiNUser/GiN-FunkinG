	...
	for child in level.get_node("note_lines").get_children():
		for varible in ["left", "down", "up", "right"]:
			if child.id == self[varible]:
				child.arrow_press.connect(play_note_anim)
				child.arrow_miss.connect(play_note_miss_anim)
				child.arrow_release.connect(note_release)
...
func change_character(new_character):
	...
	if $anim_player.has_animation("idle_loop"):
		$anim_sprite.position = $anim_player.get_animation("idle_loop").track_get_key_value(0, 0)
