...
func update_text():
	...
	for letter in text_to_add:
		...
		GlobalFunctions.add_particle(self, {
			...
			"scale" : inst_scale,
			"position" : pos + Vector2(30 * letter_pos, 0),
			"delete_on_end" : false,
...
func use():
	...
	match setting:
		...
		"time_scale":
			Engine.time_scale = SaveManager.game_data[setting]
			AudioServer.playback_speed_scale = SaveManager.game_data[setting]
