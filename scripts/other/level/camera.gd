...
func set_target(new_target, special_offset = 0):
	...
	tween_pos.tween_property(self, "position", real_pos, get_parent().stb * 4).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_EXPO)
	
	if tween_zoom:
		tween_zoom.kill()
	tween_zoom = create_tween()
	tween_zoom.tween_property(self, "smooth_zoom", real_zoom, get_parent().stb * 4).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_EXPO)
